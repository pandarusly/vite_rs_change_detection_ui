...
<template>
  <div class="flex flex-row h-screen">
    <div class="w-1/2 border-r flex ">
      ...
      <select v-model="time1" @change="select1()" class="select select-secondary w-full max-w-xs absolute bg-opacity-40" style="top: 20%; left: 15%;">
    ...
    <div class="w-1/2 flex">
      ...
      <select title="2" v-model="time2" @change="select2()" class="select select-secondary w-full max-w-xs absolute bg-opacity-40"
  ...
  <div class="absolute flex flex-col " style="bottom: 0%;left: 50%;transform: translate(-50%,5%); padding: 10px 15px;
  border-radius: 4px;
  border: 1px solid rgba(128, 128, 128, 0.5);
  z-index: 19870101;"> 
    
    <div style="padding: 2px 0px;">
      <select v-model="model_name1" class="select bg-opacity-40 select-info select-sm w-full max-w-xs ">
        <option disabled selected>模型选择</option>
        <option value="MSMCDN" >MSMCDN</option>
        <option value="FCSD">FCSD</option>
        <option value="BIT">BIT</option>
        <option value="Changformer">Changformer</option>
      </select>
    </div>
...
import {addPolygonPrimitives,addPolygonCombine,addGeojson} from "./map3d"
...
function analysis() {
  ...
  if (extent.length == 0) {
    ...
    var model_name="fp16_xiongan4class_pdacn_80k_mmtrace_512x512"
    // switch (model_name1.value) {
    //   case 'MSMCDN':
    //   model_name = 'fp16_xiongan4class_pdacn_80k_mmtrace_512x512';
    //   case 'FCSD':
    //   model_name = 'FCSD';
    //   case 'BIT':
    //   model_name = 'BIT';
    //   case 'Changformer':
    //   model_name = 'Changformer';
    //   default:
    //   model_name = '';
    // }

    if (model_name1.value=="MSMCDN") {
      model_name = 'fp16_xiongan4class_pdacn_80k_mmtrace_512x512';
    } else if (model_name1.value=="BIT") {
      model_name = 'BIT';
    } else if (model_name1.value=="FCSD") {
      model_name = 'FCSD';
    } else if (model_name1.value=="Changformer") {
      model_name = 'Changformer';
    } 
    const postdata = {
      extent: extentData,
    ...
    axios.get(url_change, {
      ...
      // addGeojson(featureCollection,graphicLayer,graphicLayerEx)
